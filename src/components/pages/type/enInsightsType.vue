...
export default {
    ...
    methods:{
        ...
        getData(){
            ...
            _this.ApiService.newTypeEn.pageTypeEn(param).then(data => {
                _this.loading = false;
                if(data && data.code === 0) {
        ...
        sureBuildType(){
            ...
            _this.ApiService.newTypeEn.addTypeEn(param).then(data => {
                if(data && data.code === 0) {
                    _this.dialogFormType = false;
                    r.message('新建成功', "success");
                    _this.buildTypeName = null;
                    _this.getData();
                }else if(data.code === 3){
        ...
        rename(row){
            let _this = this;
            if(row.name ==='News' || row.name ==='Events'){
        ...
        sureRenameType(){
            ...
            _this.ApiService.newTypeEn.updTypeEnName(param).then(data => {
                if(data && data.code === 0) {
        ...
        detele(row){
            ...
            this.$confirm('是否确认删除名称为 “' + row.name + '” 的分类?', '提示', {
                ...
                _this.ApiService.newTypeEn.removeTypeEn(param).then(data => {
                    if(data && data.code === 0) {
                        _this.getData();
                        _this.$message({
                            type: 'success',
