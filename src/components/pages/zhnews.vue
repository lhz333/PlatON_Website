<template>
    <div>
        ...
        <!--表格数据-->
        <el-table ref="selectDate" class="table-m20" v-loading="loading" :data="listData" border header-cell-style="text-align:center;background-color:#ddd;color:#333;" cell-style="padding:2px 0;" style="width:100%;" @filter-change="filterChange" :row-class-name="handleRowClass" @row-click = "handleRowClick" highlight-current-row>
            ...
            <el-table-column prop="status" label="状态" width="70" :filters="statusList" :filter-method="filterStatus" column-key="status" :filter-multiple="false">
                <template slot-scope="scope">
    ...
    export default {
        ...
        methods: {
            ...
            getType(){
                ...
                let param = {};
                let types = [];
                _this.ApiService.newTypeCn.findTypeCn(param).then(data => {
                    if (data && data.code === 0) {
            ...
            getData() {
                ...
                _this.ApiService.newsCn.findNewsCnInfo(param).then(data => {
                    _this.loading = false;
                    if (data && data.code === 0) {
            ...
            handleSizeChange(val) {
                ...
                window.sessionStorage.setItem('zhnPagesize',val);
                this.getData();
            },
            ...
            withdraw(row) {
                ...
                this.$confirm('是否确认撤回标题为 “' + row.title + '” 的新闻?', '提示', {
                    ...
                    _this.ApiService.newsCn.uptPlatNewsCnStatus(param).then(data => {
                        if(data && data.code === 0) {
                            _this.getData();
                            _this.$message({
                                type: 'success',
            ...
            release(row) {
                let _this = this;
                if(row.typeName==='活动'&&row.topImgUrl===null){
            ...
            relation(row) {
                ...
                let zhGlPagesize = window.sessionStorage.getItem('zhGlPagesize');
                if(zhGlPagesize && zhGlPagesize !== null){
                    this.pageGl.pageSize = parseInt(zhGlPagesize);
                }
            ...
            relationGetData(){
                ...
                _this.ApiService.newsCn.findNotBindPlatNewsEn(param).then(data => {
                    if(data && data.code === 0) {
            ...
            handleSizeChangeGl(val) {
                ...
                window.sessionStorage.setItem('zhGlPagesize',val);
                this.relationGetData();
            },
            ...
            sureRecord(){
                let _this = this;
                let param = {
                    cnId:_this.relationRow.id,
                    enId: _this.relationRowId
                };
                if(param.enId !==null){
            ...
            cancelRelation(row){
                ...
                this.$confirm('是否确认取消关联?', '提示', {
                    ...
                    _this.ApiService.newsCn.removeNewsCnen(param).then(data => {
                        if(data && data.code === 0) {
            ...
            record(row) {
                ...
                _this.ApiService.newsCn.findNewsCnOpertion(param).then(data => {
                    if(data && data.code === 0) {
            ...
            moveData(param) {
                var _this = this;
                _this.ApiService.newsCn.sortNewsCnUporDown(param).then(data => {
                    if(data && data.code === 0) {
        ...
        mounted() {
            let zhnPagesize = window.sessionStorage.getItem('zhnPagesize');
            if(zhnPagesize && zhnPagesize !== null){
                this.page.pageSize = parseInt(zhnPagesize);
            }
            this.getType();
            this.getData();
        }
